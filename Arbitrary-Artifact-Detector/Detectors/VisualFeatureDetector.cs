    ...
    class VisualFeatureDetector : BaseDetector, IDetector
    {
        ...
        private void FetchArtifactLibrary(Setup setup)
        {
            ...
            if (File.Exists(setup.WorkingDirectory + setup.LibraryFileName))
            {
                try
                {
                    ArtifactLibrary = ArtifactReferenceImageCache.FromFile(setup.WorkingDirectory + setup.LibraryFileName, FeatureExtractor, setup.Stopwatch, setup.LoggerFactory);
        ...
        public override DetectorResponse FindArtifact(ArtifactRuntimeInformation runtimeInformation, Setup setup, DetectorResponse previousResponse = null)
        {
            ...
            if (ArtifactLibrary == null)
            {
                ...
                try
                {
                    ArtifactLibrary = new ArtifactReferenceImageCache(setup.WorkingDirectory, FeatureExtractor, setup.GetLogger("ArtifactLibrary"), Stopwatch);
                }
                catch (Exception e)
                {
            ...
            if (setup.ShouldEvaluate)
            {
                try
                {
                    ...
                    using (StreamWriter file = new StreamWriter("output.csv", true))
                    {
                        ...
                        file.WriteLine(AADConfig.FeatureExtractorSelection + ";" + setup.ArtifactGoal + ";" + setup.Stopwatch.TimesToCSVinNSprecision() + ";" + artifactTypeFound + ";" + matchCount);
                    }
                }
                catch (Exception e)
                {
