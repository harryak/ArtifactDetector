...
artifact_results = {"sum": {"index": 999, "tp": 0, "tn": 0, "fp": 0, "fn": 0, "True": 0, "False": 0}}
    ...
    for row in plots:
        ...
        if row[9] == row[11]:
            if row[9] == "True":
                ...
                artifact_results["sum"]["tp"] += 1
            else:
                artifact_results[row[4]]["tn"] += 1
                artifact_results["sum"]["tn"] += 1
        else:
            if row[9] == "True":
                artifact_results[row[4]]["fp"] += 1
                artifact_results["sum"]["fp"] += 1
            else:
                artifact_results[row[4]]["fn"] += 1
                artifact_results["sum"]["fn"] += 1

        # Count total up
        artifact_results[row[4]][row[11]] += 1
        artifact_results["sum"][row[11]] += 1
